...
def _score_from_metrics(metrics: dict) -> dict[str, float]:
    ...
    After Bug 2 fix in evaluate.py, metrics are in correct dBFS range:
      rms_db:    typically -20 to -8 dBFS for voice
    ...
    # ── Clarity: reward -20 to -8 dBFS RMS, penalise silence and clipping ────
    # rms_norm reaches 1.0 at -8 dBFS and 0.0 at -20 dBFS (or below)
    rms_norm     = max(0.0, min(1.0, (rms_db + 20.0) / 12.0))
