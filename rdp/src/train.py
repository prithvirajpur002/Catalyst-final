...
def _safe_torch_load(ckpt_path: str) -> dict:
    ...
    try:
        ...
        # Checkpoint contains non-tensor objects (optimizer state, config dicts)
        # This is expected for raw G_*.pth files from older RVC versions
...
def train(
    ...
    # ── Launch training ───────────────────────────────────────────────────────
    # capture_output=False → loss prints live to screen/notebook
    ...
    result = subprocess.run(
        [
            ...
            "-e",  experiment_id,
            "-sr", str(sample_rate),
            "-f0", "1",               # F0 conditioning — always on for RVC
            "-bs", str(batch_size),
            "-g",  "0",
            "-te", str(epochs),
            "-se", str(save_every),
            "-pg", pretrain_g,
            "-pd", pretrain_d,
            "-l",  keep_ckpts,        # "1"=keep latest only, "0"=keep all
            "-c",  "0",               # no GPU data cache (safest for large DS)
            "-sw", "1",               # export weights after each save
            "-v",  version,
        ],
        cwd=rvc_repo,
        capture_output=False,         # live streaming to notebook output
    )
    ...
    if Path(best_raw).name.startswith("G_"):
        # Raw generator checkpoint — strip optimizer state
