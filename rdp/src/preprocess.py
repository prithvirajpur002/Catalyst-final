...
def _peak_norm(y: np.ndarray, target_db: float = -3.0) -> np.ndarray:
    ...
    # Only normalize DOWN, not up — upward normalization adds noise floor artifacts.
    if peak > target_amp:
...
def _segment(
    ...
    for s, e in ivs:
        ...
        # Check for large silence gap between previous interval end and this start
        gap_samp = s - buf_e
        ...
        if buffer_full or large_gap:
            ...
            # Start a new buffer from this interval
            buf_s, buf_e = s, e
        else:
            buf_e = e
...
def _run_demucs(src: str, tmp_dir: str, log: Logger) -> str:
    ...
    try:
        ...
        if cands and r.returncode == 0:
            return str(cands[0])
        # Log what went wrong instead of silently falling back
        ...
        # Clean up demucs temp folder for this stem
        for d in Path(tmp_dir).glob(f"*/{stem}"):
            import shutil
...
def preprocess_dataset(
    ...
    cfg = dict(MODES[mode])  # copy so we can mutate for fallback
    ...
    # ── Collect all segments first, then apply SNR filter ─────────────────────
    ...
    # ── SNR filter with one automatic fallback retry ──────────────────────────
    passing_segs, snr_vals = _filter_segments_by_snr(
        all_raw_segs, file_sr, cfg["snr_min"], log
    )

    if not passing_segs:
        # Bug 7 fix: relax SNR by 5 dB and retry once before giving up
