...
def register_model(
    ...
    # Bug 8 fix: validate model exists before touching the registry
    ...
    # Copy index file (optional — missing index is not an error)
    ...
    entry = {
        ...
        "champion":       False,  # set via mark_champion()
        "scores": {
            **scores,
            "composite": composite,
        },
    ...
    # Upsert: replace existing entry with same experiment_id
...
def mark_champion(
    ...
    for m in registry["models"]:
        ...
        if m["experiment_id"] == experiment_id:
            ...
            # Clear champion flag from all other entries
            m["champion"] = False
...
def list_models(base_dir: str = ".") -> list[dict]:
    ...
    return sorted(
        registry.get("models", []),
        key=lambda m: (
            int(m.get("champion", False)),             # champion first
            m.get("scores", {}).get("composite", 0.0), # then by score
        ),
        reverse=True,
    )
