...
def _cache_is_valid(cache_dir: str, dataset_dir: str, sample_rate: int) -> bool:
    ...
    # Verify the actual feature files are present in cache_dir
...
def _run_preprocess(rvc_repo: str, dataset_dir: str, logs_dir: str,
    ...
    r = subprocess.run(
        [sys.executable, script,
         dataset_dir,
         str(sample_rate),
         str(n_workers),   # Bug 14 fix: dynamic worker count
         logs_dir,
         "False",          # noparallel flag
         "0.3"],           # overlap (seconds)
...
def _run_hubert(rvc_repo: str, logs_dir: str, hubert_path: str, log: Logger) -> Path:
    ...
    """
    # VRAM guard (new production addition)
    ...
    r = subprocess.run(
        [sys.executable, script,
         ...
         "1",          # n_process
         "0",          # process_index
         "v2",         # RVC version (768-dim)
         logs_dir,
         hubert_path,
         "9"],         # HuBERT layer — layer 9 is best for voice conversion
        capture_output=True, text=True, cwd=rvc_repo,
    )
...
def _validate_features(logs_dir: str, feat_dir: Path, log: Logger) -> None:
    ...
    # Spot-check feature shape — all files must be (T, 768)
    npy_files = list(feat_dir.glob("*.npy"))
    if npy_files:
        dims: set[int] = set()
        for f in npy_files[:5]:  # sample first 5
            try:
                arr = np.load(str(f))
                if arr.ndim == 2:
...
def extract_features(
    ...
    # Bug 4 fix: check cache_dir, not logs_dir
    if not force and _cache_is_valid(cache_dir, dataset_dir, sample_rate):
        ...
        # Symlink or copy cached features into logs_dir so RVC training can find them
    ...
    # ── Run extraction into logs_dir (RVC needs features there for training) ─
    gt_dir = Path(logs_dir) / "0_gt_wavs"
    ...
    # ── Write cache metadata to cache_dir (Bug 4 fix) ────────────────────────
...
def _link_cache_to_logs(cache_dir: str, logs_dir: str, log: Logger) -> None:
    ...
    Uses hard links where possible (same filesystem), falls back to copy.
    """
    import shutil
    ...
    for d in dirs_to_link:
        ...
        try:
            ...
            # Symlinks may not work on some network filesystems (Kaggle /kaggle/input)
